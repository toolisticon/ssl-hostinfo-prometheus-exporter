...
function convertKeyValuePair (key, value) {
  if (value instanceof Object) {
    let result = '';
    result += Object.keys(value).map((subKey) => {
      const subValue = value[subKey];
      try {
        return `${key.toLowerCase()}_${convertKeyValuePair(subKey, JSON.parse(subValue))}`;
      } catch (ignored) {
        return `${key.toLowerCase()}_${convertKeyValuePair(subKey, subValue)}`;
      }
    });
    return result;
  } else {
...
function renderMetrics () {
  let response = `# HELP ${mozillaMetricName} Mozilla Observatory SSL stats\n`;
  response += Object.keys(resultStore).map((host) => `${normalizePrometheusDate(resultStore[host].moz)}`);
  response += `# HELP ${sslDetailsMetricName} Generic SSL Details\n`;
  response += Object.keys(resultStore).map((host) => `${normalizePrometheusDate(resultStore[host].details)}`);
  response += `# HELP ${sslExpireMetricName} Remaining Days before certificate expiry\n`;
  response += Object.keys(resultStore).map((host) => `${normalizePrometheusDate(resultStore[host].expire)}`);
  return response;
}
